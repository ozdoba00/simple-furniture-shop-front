<template></template>
<script>

import axios from 'axios'

export default {

    data() {
        return {
            recordsList: [],
            headers: ['id', 'name', 'last_name', 'email'],
            menuIcons: {'add':'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zM4 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 0110.374 21c-2.331 0-4.512-.645-6.374-1.766z" /></svg>'}
        }
    },

    methods: {
        async getRecordsList() {
            await axios
                .get("http://127.0.0.1:8000/api/admin/users", {
                    headers: {
                        Authorization: `Bearer 5|0mlVPO1tzmH9b5Ed2mxyyR9LIiUEvSzfTS2Ibutq`,
                        token: '5|0mlVPO1tzmH9b5Ed2mxyyR9LIiUEvSzfTS2Ibutq'
                    }
                })
                .then(res => {
                    let data = res.data
                    this.$emit('recordsList', data);
                });
        },

        getHeaders()
        {
            return this.headers;
        },

        getMenuIcons()
        {
            return this.menuIcons;
        }
    },

    async mounted() {
        await this.getRecordsList();
    },
}


</script>
